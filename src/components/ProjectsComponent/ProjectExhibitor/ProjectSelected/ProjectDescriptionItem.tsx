...
const ProjectDescriptionItemComponent = ({ text, logo, type }: ProjectDescriptionItemProps): React.ReactNode => {
  return (
    <Box
      sx={(theme: Theme) => ({
        position: 'relative',
        backgroundColor: type === projectItemTypeEnum?.feature ? theme?.custom?.white : theme?.palette?.primary?.main, 
        borderRadius: '1em',
        textAlign: 'center',
        color: type !== projectItemTypeEnum?.feature ? theme?.custom?.white : theme?.palette?.primary?.main,
        fontSize: '0.8em',
        p: '1em',
        m: '1em auto',
        width: { xs: '100%', sm: '100%' },
        ...
        '&::after': {
          ...
          position: 'absolute',
          bottom: '-0.8em',
          left: type === projectItemTypeEnum?.feature && 0,
          right: type === projectItemTypeEnum?.benefit && 0,
          rotate: `z ${type === projectItemTypeEnum?.feature ? '30deg' : '60deg' }`,
          width: 0,
          height: 0,
          borderLeft: '0.5em solid transparent',
          borderRight: '1.5em solid transparent',
          borderTop: `1.5em solid ${type === projectItemTypeEnum?.feature ? theme?.custom?.white : theme?.palette?.primary?.main}`,
        },
      })}
    >
      <li style={{ listStyle: 'none', display: 'flex', alignItems: 'center', gap: '0.5em' }}>
        {logo && (
          type === projectItemTypeEnum?.benefit ? (
            <Box
              sx={(theme: Theme) => ({
                width: '2em',
                height: '2em',
